#!/bin/sh
#
# Start/Stop the SpringBoot server.
#
...
waitForPid () {
  ...
  while [ 1 -eq 1 ]; do
    ...
    if [ $? -eq 1 ]; then
      echo "SpringBoot server PID ${MTRPID} exited"
      return 1
    fi
  ...
  echo "SpringBoot server PID ${MTRPID} did not exit."
  return 0;
}
...
doStopSignal () {
  ...
  if [ -f "${SERVER_PID}" ] ; then
    ...
    if [ "x${MTRPID}" = "x" ] ; then
      echo "SpringBoot pid file was empty: ${SERVER_PID}"
      exit 127
    fi
    ...
    echo "SpringBoot server not running (no pid file found: ${SERVER_PID})"
  fi
}
...
if [ -f "${SERVER_PID}" ] ; then
  ...
  if [ ! "x${MTRPID}" = "x" ] ; then
    ...
    if [ $? -eq 1 ]; then
      ...
      echo "SpringBoot server is already running (pid ${MTRPID})."
      exit 0
    fi
  fi
fi
...
# Start the server
echo "Booting the SpringBoot server (Using JAVA_OPTS=${JAVA_OPTS})..."

java ${JAVA_OPTS} -cp ${CLASSPATH} com.github.cnkeep.WebApplication > ${SERVER_LOG} 2>&1 &
...
case "$1" in
  start)
    ...
    doStart
    echo "SpringBoot server booted."
    ;;
  stop)
    echo "Stopping SpringBoot server..."
    doStop
    echo "SpringBoot server is stopped."
    ;;
  *)
